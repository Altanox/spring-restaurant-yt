...
public class FileSystemStorageService implements StorageService {
    ...
    // either provide a value or default to "uploads" dir in the root of the project
    @Value("${app.storage.location:uploads}")
    private String storageLocation;

    private Path rootLocation;

    // Annotation used on a method that needs to be executed after dependency injection is done to perform any initialization
    ...
    @Override
    public String store(MultipartFile file, String filename) throws StorageException {
        try {
            if (file.isEmpty()) {
                throw new StorageException("Cannot save to an empty file");
            }
    ...
    public Optional<Resource> loadAsResource(String filename) {
        try {
            ...
            if (resource.exists() && resource.isReadable()) { // in the course, there was || here
